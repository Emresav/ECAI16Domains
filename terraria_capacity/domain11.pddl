...
;; in this example, we now have a different type of wood: Mahogany Wood, which will be used to make Ivy_chests..
;; we can cut Mahogany tree in a forest somwhere really far.......
...
:condition (and (at start (available))
                ...
                (at start (<= ?cob 60))
                (at start (<= (+ ?cob (+ ?iron (+ ?chain_found (+ ?lead ?platinum ) ) ) )  40) )
                (at end (>= (stock ?c) 0))
                (at end (>= (stock ?i) 0))
                (at end (>= (stock ?ch) 0))
                (at end (>= (stock ?lb) 0))
