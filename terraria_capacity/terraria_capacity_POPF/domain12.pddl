...
(:durative-action cut_mahogany_tree_3unit
...
:effect (and    (at start (not (available)))
                (at start (increase (stock ?a) 3))
                (at end (available))
))

(:durative-action cut_mahogany_tree_10unit
:parameters(?a - mahogany_wood ?loc1 - location)
:duration (= ?duration 2)
:condition (and (at start (available))
                (at start (can_lumber_mahogany ?loc1))
...
(:durative-action cut_mahogany_tree_40unit
:parameters(?a - mahogany_wood ?loc1 - location)
:duration (= ?duration 2)
:condition (and (at start (available))
                (at start (can_lumber_mahogany ?loc1))
...
(:durative-action produce_ivy_chests_1unit
...
:effect (and    (at start (not (available)))
                (at start (increase (stock ?c) 1) )
                (at start (decrease (stock ?mw) 8 ) )
                (at start (decrease (stock ?lb) 2 ) )
                (at start (decrease (stock ?i) 2 ) )
                (at end (available))
))

(:durative-action produce_ivy_chests_10unit
:parameters(?c - ivy_chest ?mw - mahogany_wood ?lb - lead_bar ?i - iron_bar ?loc1 - location)
:duration (= ?duration 5)
:condition (and (at start (available))
                (at start (ready_workbench)) ; made on sawmill
...
:effect (and    (at start (not (available)))
                (at start (increase (stock ?c) 10) )
                (at start (decrease (stock ?mw) 80 ) )
                (at start (decrease (stock ?lb) 20 ) )
...
:condition (and     (at start (> (stock ?b) 14) )
                    (at start (available))
                    (at end (>= (stock ?b) 0) )  
                     )
:effect (and        
...
:effect (and        
                    ...
                    (at start (decrease (stock ?c) 14))
                    (at end (available)) ) )

(:durative-action place_swords
:parameters (?s - platinum_sword)
:duration (<= ?duration 0.5)
:condition (and     (at start (> (stock ?s) 14) )
                    (at start (available))
                    (at end (>= (stock ?s) 0) )  
                     )
:effect (and        
