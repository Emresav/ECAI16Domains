...
:parameters(?i - itemI ?e - itemE ?j - itemJ ?k - itemK ?n - itemN  ?m - itemM ?f - itemF ?loc1 - location)
:control (?batchsize_E ?batchsize_I ?batchsize_J ?batchsize_K ?batchsize_N ?batchsize_M ?batchsize_F - number)
:duration (= ?duration 1)
:condition (and (at start (>= ?batchsize_I 0))
                ...
                (at start (>= ?batchsize_N 0))
                (at start (<= ?batchsize_N 80))
                (at start (>= ?batchsize_M 0))
                (at start (<= ?batchsize_M 80))
                (at start (>= ?batchsize_F 0))
                (at start (<= ?batchsize_F 80))
                (at start (available))
...
:effect (and    (at start (not (available)))
                ...
                (at start (increase (stock ?f) ?batchsize_F))                                
                (at end (available))
)
)
