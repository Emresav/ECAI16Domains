...
:parameters(?i - itemI ?e - itemE ?j - itemJ ?k - itemK ?l - itemL ?n - itemN ?loc1 - location)
:control (?batchsize_E ?batchsize_I ?batchsize_J ?batchsize_K ?batchsize_L ?batchsize_N - number)
:duration (= ?duration 1)
:condition (and (at start (>= ?batchsize_I 0))
                ...
                (at start (>= ?batchsize_L 0))
                (at start (<= ?batchsize_L 40))
                (at start (>= ?batchsize_N 0))
                (at start (<= ?batchsize_N 40))
                (at start (available))
...
:effect (and    (at start (not (available)))
                ...
                (at start (increase (stock ?l) ?batchsize_L))
